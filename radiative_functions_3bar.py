...
OLR_hybrid_FIX=np.load("OLR_200_FIX_3bar.npy")
OLR_hybrid_FIX = np.log10(OLR_hybrid_FIX)
water_frac_multi_new=np.load("Atmo_frac_200_FIX_3bar.npy")
fH2O_new = np.load("fH2O_200_FIX_3bar.npy")
...
@jit(nopython=True) 
def my_interp_3bar(Tsurf,Te,PH2O,PCO2):  
...
@jit(nopython=True)
def my_water_frac_3bar(Tsurf,Te,PH2O,PCO2):  
...
@jit(nopython=True)
def my_fH2O_3bar(Tsurf,Te,PH2O,PCO2):  
...
@jit(nopython=True) 
def correction_3bar(Tsurf,Te,PH2O,PCO2,rp,g,CO3,PN2,MMW,PO2): 
    ...
    atmo_fraction = my_water_frac_3bar(Tsurf,Te,PH2O,PCO2)
    if (atmo_fraction == 1.0) or (PCO2<0): # if no liquid water ocean, all CO2 in atmosphere and same result as before
        return [my_interp_3bar(Tsurf,Te,PH2O,PCO2),PCO2,0.0,0.0,0.0]
    else:
        ...
        true_OLR =  my_interp_3bar(Tsurf,Te,PH2O,EM_ppCO2_o*1e5)
