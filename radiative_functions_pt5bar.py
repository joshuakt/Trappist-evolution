...
OLR_hybrid_FIX=np.load("OLR_200_FIX_pt5bar.npy")
OLR_hybrid_FIX = np.log10(OLR_hybrid_FIX)
water_frac_multi_new=np.load("Atmo_frac_200_FIX_pt5bar.npy")
fH2O_new = np.load("fH2O_200_FIX_pt5bar.npy")
...
### Function for OLR grid interpolation
@jit(nopython=True) 
def my_interp_pt5bar(Tsurf,Te,PH2O,PCO2):  
    ...
    if Te<=np.min(Te_grid):
        ...
        Te_index = len(Te_grid) - 2 
    else:
        for i in range(1,len(Te_grid)): 
    ...
    if PH2O <= np.min(P_H2O_grid_new):
        ...
        H2O_index = len(P_H2O_grid_new) - 2 
    else:
    ...
    if PCO2 <= np.min(P_CO2_grid_new):
        ...
        CO2_index = len(P_CO2_grid_new) - 2
    else:
    ...
    answer = C*(1-qd) + qd * DC
    if Tsurf < np.min(T_surf_grid):
         return (Tsurf/np.min(T_surf_grid)) * 10**answer # Extrapolation for cold surface temperatures (rarely used)
         #return (Tsurf/np.min(T_surf_grid))**4 * 10**answer #Alternative extrapolation for cold surface temperatures
    elif Tsurf > np.max(T_surf_grid):
         return (Tsurf/np.max(T_surf_grid))**4 * 10**answer # Extrapolation for hot surface temperatures (rarely used)
    else:
        return 10**answer
        
### Function for atmosphere-ocean water partition grid
@jit(nopython=True)
def my_water_frac_pt5bar(Tsurf,Te,PH2O,PCO2):  
...
### Function for stratospheric water vapor grid interpolation
@jit(nopython=True)
def my_fH2O_pt5bar(Tsurf,Te,PH2O,PCO2):  
    ...
    #improved @ cold temperatures
    if Tsurf < np.min(T_surf_grid): ## Extrapolation for low surface temperatures (rarely used)
        if Tsurf < Te:
            return ((np.exp(43.494 - 6545.8/(Te-273.15+278))/((Te-273.15+868)**2)))/(1e5 + PCO2) ### ignoring PN2 variation here!!
        else:
            frst = (Tsurf/np.min(T_surf_grid))**3 * 10**answer 
            snd = ((np.exp(43.494 - 6545.8/(Tsurf-273.15+278))/((Tsurf-273.15+868)**2)))/(1e5 + PCO2) ### ignoring PN2 variation here!!   
            if frst<snd:
                return frst
            else:
                return snd 
...
@jit(nopython=True) 
def correction_pt5bar(Tsurf,Te,PH2O,PCO2,rp,g,CO3,PN2,MMW,PO2): 
    ...
    atmo_fraction = my_water_frac_pt5bar(Tsurf,Te,PH2O,PCO2)
    if (atmo_fraction == 1.0) or (PCO2<0): # if no liquid water ocean, all CO2 in atmosphere and same result as before
        return [my_interp_pt5bar(Tsurf,Te,PH2O,PCO2),PCO2,0.0,0.0,0.0]
    else:
        ...
        if cCon < CO3: #quick fix to ensure no more CO3(2-) than total carbon in total atmo-ocean system!
            CO3 = 0.9999*cCon
        ...
        EM_pH_o=-np.log10(EM_H_o) ##  ocean pH
        EM_hco3_o=ALK-2*CO3 ## bicarbonate molality
        EM_co2aq_o=( EM_hco3_o*EM_H_o/(10**-pK1) ) ## aqueous CO2 molality
        EM_ppCO2_o = EM_co2aq_o /H_CO2 ## atmospheric pCO2
        DIC_check = EM_hco3_o + CO3 + EM_co2aq_o
        true_OLR =  my_interp_pt5bar(Tsurf,Te,PH2O,EM_ppCO2_o*1e5)
        return [true_OLR,EM_ppCO2_o*1e5,EM_pH_o,ALK,Mass_oceans_crude]
