...
#print(np.where(np.isnan(fH2O_new)))
## fix a few nans in the fH2O frac
fH2O_new[22,0,28,23] = fH2O_new[22,1,28,23] 
fH2O_new[22,0,29,22] = fH2O_new[22,1,29,22] 
fH2O_new[22,0,33,20] = fH2O_new[22,1,33,20]
fH2O_new[27,0,32,15] = fH2O_new[27,1,32,15]
fH2O_new[27,0,32,22] = fH2O_new[27,1,32,22]
fH2O_new[27,0,33,19] = fH2O_new[27,1,33,19]
#print(np.where(np.isnan(fH2O_new)))
...
def correction(Tsurf,Te,PH2O,PCO2,rp,g,CO3,PN2,MMW,PO2): 
    ...
    if (atmo_fraction == 1.0) or (PCO2<0): # if no liquid water ocean, all CO2 in atmosphere and same result as before
        return [my_interp(Tsurf,Te,PH2O,PCO2),PCO2,0.0,0.0,0.0]
    else:
