...
@jit(nopython=True)
def H2O_partition_function2( yH2O_liq,Mliq,Mcrystal,rp,g,kH2O,MMW): #partition H2O between magma ocean and atmosphere
    if (Mliq >0)or(Mliq>0):
        initialize_fast = np.array(0.01)
        ace1 =  nelder_mead(f_for_optim2, x0=initialize_fast, bounds=np.array([[0.0], [1.0]]).T, args = (kH2O,Mcrystal,Mliq,rp,yH2O_liq,g,MMW), tol_f=0.0001,tol_x=0.0001, max_iter=1000)
        FH2O = ace1.x[0]
        Pressure_surface = (FH2O / 3.44e-8)**(1.0/0.74)
        #FH2O = FH2Oeh/(1+FH2Oeh)
    else:
